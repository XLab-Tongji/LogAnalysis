...
log_file = ['../5G/5G_normal_train.log','../5G/5G_normal_val.log','../5G/5G_normal_test.log','../5G/5G_abnormal.log']
...
log_list = []
with open('../5G/Union.log','r') as file:
...
temp7_list = []
temp8_list = []
path = '../5G/LogClusterResult-5G/logvalue/' + 'all.txt'
outfile = './output.txt'
for i in range(0, len(log_list)):
    with open(outfile, 'w') as file:
        file.write('3 ' + str(i / (len(log_list) - 1) * 100))
        templog = []
...
for i in range(0,4):
    log_key_list = []
    path = '../5G/LogClusterResult-5G/logkey/'+log_key_file[i]
    ...
    for j in range(0,max(log_key_list)):
        path = '../5G/LogClusterResult-5G/logvalue/'+log_value_dir[i] + str(j+1)+'.txt'
        with open(path, mode='w', encoding='utf-8') as f:
            for t in log_func[j]:
                if i==0:
                    index = t
